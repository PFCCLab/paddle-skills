...
- 将本轮调试产生的原始日志和关键观测结果统一保存到 `.paddle-pilot/debug-logs/` 目录，便于后续对比与归档。
...
- 报告以独立文件形式存放在 `.paddle-pilot/debug-analysis/` 目录，按问题编号或日期命名，便于团队检索。
- 归因时可结合以下典型维度进行简要说明：
  - 接口 / 形状 / dtype：
...
- 对已经定位并（准备）修复的问题：
  ...
  - 将最终结论与关键证据沉淀到 `.paddle-pilot/debug-analysis/` 中对应的问题文档，保证与代码和用例信息一致。
