...
- 常用观测方式：
  ...
  - 对比法：
    - 对同一逻辑分别在 CPU / GPU、单卡 / 多卡上运行，比较中间结果差异。
    - 通过简化模型或更小 batch 验证问题是否依赖规模或并行度。
  - 版本与环境信息：
    - 记录 `paddle.__version__`、CUDA/CUDNN 版本、驱动信息等，保证之后能在相同环境下复现。
- 每完成一次带观测点的复现：
  - 基于运行时数据**排除不成立的假设**，收敛到少数更可疑的根因；
  - 在更窄的范围内继续加观测点，逐步缩小问题所在的模块 / 算子 / 配置。
- 将本轮调试产生的原始日志和关键观测结果统一保存到 `.paa/debug-logs/` 目录，便于后续对比与归档。
...
- 报告以独立文件形式存放在 `.pad/debug-analysis/` 目录，按问题编号或日期命名，便于团队检索。
- 归因时可结合以下典型维度进行简要说明：
  - 接口 / 形状 / dtype：
  ...
  - 分布式 / 多卡：
    - 写清单卡是否正常、多卡从哪个 rank / 哪一步开始出现差异或 hang，以及是否存在只在部分 rank 执行的逻辑。
...
- 对已经定位并（准备）修复的问题：
  ...
  - 将最终结论与关键证据沉淀到 `.pad/debug-analysis/` 中对应的问题文档，保证与代码和用例信息一致。
